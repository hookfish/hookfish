...
A server-rendered operator frontend can mount `@hookfish/client`. This Hono
facade reads the broker credential in the server process and issues an
ephemeral `HttpOnly`, `SameSite=Strict` operator cookie. It exposes only safe
connection-management operations and public OAuth callbacks; it does not proxy
the raw API or provider-token retrieval.

The operator cookie is a CSRF boundary, not user authentication. Keep the
frontend on loopback or require application authentication, SSO, or another
access-control layer for a hosted operator tool. When a trusted proxy terminates
TLS, forward the original protocol so the facade adds the `Secure` cookie flag.
