...
| Variable | Purpose |
| --- | --- |
| `HOOKFISH_BACKEND_URL` | Hookfish API origin used by the packaged dashboard |
| `FRONTEND_HOST` / `FRONTEND_PORT` | Packaged dashboard bind address |
| `HOOKFISH_BASE_PATH` | Optional constant subtree for the Better Auth resolver; defaults to `global` |
| `BETTER_AUTH_SECRET` | Better Auth session signing secret |
| `BETTER_AUTH_DATABASE` | Better Auth SQLite database path |
...
The packaged frontend uses `HOOKFISH_API_KEY` as the root signing key for the
capability produced after a Better Auth session resolves to a base path. The
inspector uses the same variable as its local broker key and defaults to `test`
when it is unset.
