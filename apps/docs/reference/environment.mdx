...
| `BROKER_API_KEY` | For authenticated API operations | Root API credential and scoped-token signing key |
...
<Warning>
  ...
  Rotating `BROKER_API_KEY` invalidates every scoped broker token.
</Warning>
...
| `HOOKFISH_API_KEY` | Inspector's local broker key; defaults to `test` |
