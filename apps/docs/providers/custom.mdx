...
Built-in providers can also contribute a caller-side client factory. That is
how a static GitHub provider returns Octokit and a fixed MCP provider returns a
connected MCP client. A custom provider without a caller-side factory remains
fully usable through `connections.access()`.

Provider methods normalize only connection-acquisition errors. Once a method
returns an official or custom SDK client, that client keeps its native API and
error behavior.
