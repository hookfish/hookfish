...
The operation needs `BROKER_API_KEY`, `OAUTH_ENCRYPTION_KEY`, or provider
...
Open `/api` for Swagger UI or request `/api/openapi.json`. The document reflects
the configured organization-routing, provider-management, and Swagger settings
for that deployment.
