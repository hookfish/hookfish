...
`hookfish dev` is an alias. The command serves the packaged dashboard behind a
loopback operator BFF. It forwards only explicit safe operations and keeps
`HOOKFISH_API_KEY` in the CLI process.
