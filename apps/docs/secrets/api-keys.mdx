...
The separate `hookfish.secrets` API remains available for arbitrary encrypted
values that do not need a provider-backed connection lifecycle. Prefer
`connections.setSecret` when callers should consume OAuth and static
credentials through one interface.
