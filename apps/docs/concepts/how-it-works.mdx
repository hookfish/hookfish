...
In self-hosted mode, Hookfish owns OAuth state and encrypted credential
storage. A [managed OAuth backend](/deployment/managed-oauth-backends) can
instead delegate OAuth state, provider tokens, refresh, and revocation to a
service such as Arcade. Hookfish retains the same paths, tenant isolation, and
HTTP contract in both modes.
...
The following lifecycle describes self-hosted `{ db, providers }` mode. A
managed backend implements the equivalent operations while owning its OAuth
