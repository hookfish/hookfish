...
`connections.access()` is server-only. An application endpoint may return safe
connection metadata or an authorization URL, but it must never return a usable
provider credential. Verify the application session, apply its authorization
policy, and derive canonical Hookfish paths from trusted session data.
