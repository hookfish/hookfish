...
Hookfish includes a static dashboard for viewing and disconnecting connections.
Treat it as an operator tool. Build user-facing connection
...
browser session storage. Connection lists contain metadata only.
...
true`. The facade exposes metadata and disconnect, but never connection access,
secret writes, vault values, callbacks, or administration.
