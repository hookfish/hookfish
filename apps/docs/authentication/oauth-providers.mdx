...
When a token endpoint rejects a refresh, include its HTTP status and OAuth error
identifier in `ProviderRequestError`:

```ts
throw new ProviderRequestError('Example token refresh failed.', {
  status: response.status,
  oauthError: payload.error,
})
```

Hookfish treats only `invalid_grant` as a terminal refresh failure and clears
the unusable credentials. Rate limits, 5xx responses, malformed responses, and
other transient failures leave stored credentials intact so a later request
can retry.
...
<Tip>
  ...
  `ProviderRequestError` for upstream failures. Preserve the upstream `status`
  and `oauthError` when they are available.
</Tip>
