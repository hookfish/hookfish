...
import { hookfishFor } from '@/lib/hookfish'
import {
  addLocalFolder,
  LOCAL_FOLDERS_KEY,
  readLocalFolders,
...
function isClientContext(value: unknown): value is ClientContext {
  ...
  return (
    ...
    (basePath === null || typeof basePath === 'string') &&
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (selectedProvider.authentication === 'secret') {
        await setConnectionSecret(path, secret, clientApiUrl)
        await onSaved()
        onOpenChange(false)
      } else {
...
export function OAuthConnections({
  ...
  const { clientApiUrl, signInUrl } = useHookfishBrowserConfig()
  const hookfish = hookfishFor(clientApiUrl)
  ...
  const context = useQuery({
    queryKey: ['hookfish', 'context'],
    queryFn: async ({ signal }) => {
      const response = await fetch(`${clientApiUrl}/context`, { signal })
      if (!response.ok) {
        throw new Error(
          response.status === 401
            ? 'Sign in to access Hookfish connections.'
            : 'Could not resolve the Hookfish access context.',
        )
      }
      const value: unknown = await response.json()
      if (!isClientContext(value)) {
        throw new Error('The Hookfish access context is invalid.')
      }
      return value
    },
  })
  const basePath = context.data?.basePath ?? ''
  const canonicalCurrentPath = basePath
    ? joinConnectionPath(basePath, currentPath)
    : ''
  const relativePath = (path: string) =>
    path === basePath ? '' : path.slice(basePath.length + 1)
  const connections = hookfish.useConnections(
    {
      ...(view === 'tree' && canonicalCurrentPath
        ? { namespace: canonicalCurrentPath }
        : {}),
  ...
  const directory = useMemo(
    ...
    [connectionItems, canonicalCurrentPath, localFolders, activePending],
  )
  const visiblePending =
    view === 'tree'
      ? directResources(activePending, canonicalCurrentPath)
      : activePending
    ...
    >
      <Card className="h-full min-h-0 min-w-0 gap-0 rounded-none border-x-0 border-b-0 border-t-2 border-t-primary py-0">
        <CardContent className="grid min-h-0 min-w-0 flex-1 content-start gap-8 overflow-x-hidden overflow-y-auto px-4 pb-8 md:px-8">
          ...
          {context.isPending || connections.isPending || providers.isPending ? (
            <LoadingItems />
          ) : null}
          {context.error ? (
            <Alert variant="destructive">
              <AlertTitle>Could not load access context</AlertTitle>
              ...
              {view === 'tree'
                ? directory.folders.map((folder) => (
                    <Item asChild variant="outline" key={folder.path}>
                      <button
                        type="button"
                        onClick={() => onNavigate(relativePath(folder.path))}
                      >
                        <ItemMedia variant="icon">
                          <FolderIcon />
                        </ItemMedia>
      ...
      <AddFolderDialog
        ...
        open={folderOpen}
        currentPath={canonicalCurrentPath}
      ...
      <AddResourceDialog
        ...
        open={resourceOpen}
        currentPath={view === 'tree' ? canonicalCurrentPath : basePath}
