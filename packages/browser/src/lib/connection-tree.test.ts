...
describe('connection tree', () => {
  it('joins only non-empty path segments', () => {
    expect(joinConnectionPath('global', '')).toBe('global')
    expect(joinConnectionPath('', 'github')).toBe('github')
    expect(joinConnectionPath('global', 'github')).toBe('global/github')
  })
