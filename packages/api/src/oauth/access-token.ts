...
export async function authenticateAccessToken(
  ...
  if (isApplicationAccessToken(token)) {
    try {
      const grant = await verifyApplicationAccessToken(rootApiKey, token, now)
      return {
        kind: 'scoped',
        name: `application.${toBase64Url(new TextEncoder().encode(grant.tenantId)).slice(0, 96)}`,
        scopes: normalizeResourceScopes(grant.scopes),
